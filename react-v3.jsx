...
import { ArrowRight, Instagram, Zap, Globe, MousePointer2, ChevronDown, Camera, Upload, Plus, Clock, Coffee, FileText, Music, MapPin, Calendar, Heart, Hexagon } from 'lucide-react';

// --- Custom Components ---

// 1. Marquee (Teks Berjalan) - UPDATED TEXT
...
// 3. Photo Fan Deck (Layout Kartu Menumpuk)
const PhotoDeck = () => {
  const cards = [
...
// 4. Logo Placeholder Component (NEW)
...
const App = () => {
  ...
  const [unlockedSections, setUnlockedSections] = useState([]); // State untuk nav items yang sudah terbuka
  ...
  // Section Observer for Dynamic Navbar (UNLOCK SYSTEM)
  useEffect(() => {
    const sections = ['journey', 'social', 'highlights', 'memories'];
    
    const observer = new IntersectionObserver((entries) => {
      entries.forEach(entry => {
        if (entry.isIntersecting) {
          setUnlockedSections(prev => {
             // Jika section belum ada di list unlocked, tambahkan
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-white font-sans selection:bg-lime-400 selection:text-black overflow-x-hidden cursor-none">
      ...
      {/* Navigation (UPDATED: Unlocks on Scroll) */}
      <nav className="fixed w-full z-40 px-6 py-6 flex justify-between items-center mix-blend-difference">
        ...
        <button className="bg-white text-black px-6 py-2 rounded-full font-bold hover:bg-lime-400 transition-colors uppercase text-sm flex items-center gap-2 hover:scale-105 active:scale-95">
      ...
      {/* BANNER MARQUEE (UPDATED TEXT) */}
      <div className="my-10 rotate-1">
        <Marquee text="WORKSHOP • PKM • BELAJAR • TEMA • EDULIT • " />
      </div>
            ...
            <div className="md:col-span-8 relative group cursor-pointer">
              <RevealCard className="h-full">
                <div className="h-[500px] rounded-3xl relative overflow-hidden border border-neutral-800 group-hover:border-lime-400 transition-all duration-500">
                   ...
                   <div className="absolute top-28 right-8 opacity-20 group-hover:opacity-100 transition-opacity">
      ...
      {/* HIGHLIGHTS SECTION (VERTICAL PARALLAX GRID) */}
                 ...
                 <RevealCard key={`left-${idx}`} className="w-full group">
                    <div className={`relative bg-neutral-800 p-4 pb-12 rounded-sm shadow-xl transform ${item.rotate} transition-transform duration-500 group-hover:rotate-0 group-hover:scale-105 group-hover:z-10 border border-neutral-700 hover:border-lime-400`}>
                      ...
                      <div className="aspect-[4/5] overflow-hidden mb-4 bg-black relative">
                        ...
                        <div className="absolute top-2 left-2 bg-black/70 text-lime-400 text-xs font-bold px-2 py-1 uppercase tracking-widest border border-lime-400/30">
                          {item.tag}
                        </div>
                      </div>
