...
const Highlights = ({ scrollY }) => {
  ...
  return (
    <section id="highlights" className="py-24 bg-neutral-950 relative overflow-hidden">
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-secondary rounded-full blur-[150px] opacity-5 pointer-events-none"></div>
      <div className="w-full px-6 md:px-12 relative z-10 max-w-screen-2xl mx-auto">
        <RevealCard>
          <div className="flex flex-col md:flex-row justify-between items-end mb-24 gap-6 border-b border-neutral-800 pb-8">
            <h2 className="text-4xl md:text-7xl font-black uppercase tracking-tighter leading-none">{highlightsStaticData.title.prefix} <br/> <span className="text-transparent stroke-white stroke-1" style={{ WebkitTextStroke: '2px #F59E0B' }}>{highlightsStaticData.title.highlight}</span></h2>
            <div className="flex flex-col items-end"><p className="text-secondary font-mono text-xl mb-2">{highlightsStaticData.rewind}</p><p className="text-neutral-400 max-w-xs text-right text-sm">{highlightsStaticData.desc}</p></div>
          </div>
        </RevealCard>
           ...
           <div className="space-y-12 md:space-y-24 pt-0 md:pt-12">
             {leftColumn.map((item, idx) => (
               <RevealCard key={`left-${idx}`} className="w-full group">
                  <div className={`relative bg-neutral-800 p-4 pb-12 rounded-sm shadow-xl transform ${item.rotate} transition-transform duration-500 group-hover:rotate-0 group-hover:scale-105 group-hover:z-10 border border-neutral-700 hover:border-secondary`}>
                    <LogoPlaceholder />
                    <div className="aspect-[4/5] overflow-hidden mb-4 bg-black relative"><img src={item.img} alt={item.title} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" /><div className="absolute top-2 left-2 bg-black/70 text-secondary text-xs font-bold px-2 py-1 uppercase tracking-widest border border-secondary/30">{item.tag}</div></div>
